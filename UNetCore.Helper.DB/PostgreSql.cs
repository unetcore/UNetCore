...
namespace UNetCore.Helper.DB
{
    public class PostgreSql : DbBase
    {
        ...
        {
            //DapperExtensions默认是用SqlServer的标记，这边使用PostgreSql的特殊语法标记，需要在初始化时给予指定。
            //DapperExtensions是静态的，全局只能支持一种数据库 
            DapperExtensions.SqlDialect = new Sql.PostgreSqlDialect();
        }

        /// <summary>
        /// 创建连接对象。子类需要重写，根据自己的数据库类型实例化自己的数据库连接对象(支持IDbConnection接口)。
        /// Dapper可以在所有Ado.net Providers下工作，包括sqlite, sqlce, firebird, oracle, PostgreSql, PostgreSQL and SQL Server。
        /// </summary>
        /// <returns></returns>
        protected override IDbConnection CreateConnection()
        {
            //todo 可用Nuget下载支持PostgreSql的程序包，然后就可以了。我本地没有安装PostgreSql，不能测试，先注释了，反正就是这么简单，之前也用过PostgreSql。
            IDbConnection connection = new NpgsqlConnection(ConnectionString);// PostgreSql.Data.PostgreSqlClient
            return connection;
            // throw new Exception("//todo:使用前请先下载支持PostgreSql的程序包PostgreSql.Data");
        }
        #endregion
        ...
        /// <summary>
        /// PostgreSql版。获取分页查询sql，支持多语句查询，详见otherSql参数
        /// (这边只负责组合分页sql)
        /// </summary>
        ...
        /// <summary>
        /// PostgreSql版。执行sql前对sql的处理事件。可以检查sql安全性，也可以按不同数据库sql写法做调整。
        /// </summary>
        public override string CheckSql(string strSql)
        {
            //sqlserver、PostgreSql是用@，oracle的参数是用冒号
            return strSql.Replace(':', '@');//如果你们项目确定是用一种数据库写法，以后不会变，则直接return strSql;

            //if (如果有可疑sql(PostgreSql版)) 则记录日志并抛异常
        }
